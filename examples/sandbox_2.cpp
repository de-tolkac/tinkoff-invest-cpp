...
int main() {
    ...
    SandboxAccount account = sandboxRest.Register(err, BrokerAccountType::Tinkoff);
    if (err) {
    ...
    UserAccountList userAccounts = sandboxRest.Accounts(err);
    if (err) {
    ...
    PlacedOrder limitOrder = sandboxRest.LimitOrder(err, account.brokerAccountId, "BBG005DXJS36", 10, OperationType::Buy, 1.0);
    if (err) {
    ...
    PlacedOrder marketOrder = sandboxRest.MarketOrder(err, account.brokerAccountId, "BBG005DXJS36", 10, OperationType::Buy);
    if (err) {
    ...
    OrderList orders = sandboxRest.Orders(err, account.brokerAccountId);
    if (err) {
    ...
    PortfolioPositionList positions = sandboxRest.PortfolioPositions(err, account.brokerAccountId);
    if (err) {
    ...
    CurrencyPositionList currencies = sandboxRest.PortfolioCurrencies(err, account.brokerAccountId);
    if (err) {
    ...
    PortfolioInfo portfolio = sandboxRest.Portfolio(err, account.brokerAccountId);
    if (err) {
    ...
    MarketInstrumentList stocks = sandboxRest.Stocks(err);
    if (err) {
    ...
    MarketInstrumentList bonds = sandboxRest.Bonds(err);
    if (err) {
    ...
    MarketInstrumentList etfs = sandboxRest.ETFs(err);
    if (err) {
    ...
    MarketInstrumentList currenciesList = sandboxRest.Currencies(err);
    if (err) {
    ...
    OrderBook orderBook = sandboxRest.Orderbook(err, "BBG005DXJS36", 10);
    if (err) {
    ...
    CandleList candles = sandboxRest.Candles(err, "BBG005DXJS36", "2019-08-19T18:38:33+03:00", 
    ...
    SearchMarketInstrument instrument = sandboxRest.GetIntsrumentByFIGI(err, "BBG005DXJS36");
    if (err) {
    ...
    MarketInstrumentList instrumentList = sandboxRest.GetInstrumentByTicker(err, "TCS");
    if (err) {
    ...
    OperationList operations = sandboxRest.Operations(err, account.brokerAccountId, "BBG005DXJS36", 
